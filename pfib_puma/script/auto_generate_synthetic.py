...
def lognormal_random_spheres(shape, voxel_size_nm, target_porosity,
                             mu_log, sigma_log, nsamples=8,
                             d_min_nm=30, d_max_nm=1800,
                             allow_intersect=True, seed=None):
    """
    Generate composite spheres from a continuous log-normal PSD.
    - mu_log, sigma_log: mean and std of ln(diameter_nm)
    - nsamples: how many sphere generations to split into
    - d_min_nm, d_max_nm: truncate sphere sizes to this range (nm)
    """
    if seed is not None:
        np.random.seed(seed)
        random.seed(seed)
    ...
    por_share = target_porosity / nsamples

    for i in range(nsamples):
        # sample a diameter from log-normal
        d_nm = np.random.lognormal(mean=mu_log, sigma=sigma_log)
        d_nm = np.clip(d_nm, d_min_nm, d_max_nm)

        d_vox = nm_to_vox(d_nm, voxel_size_nm)
        d_vox = max(1, int(round(d_vox)))
        tmp = generate_random_spheres(
            shape=shape, diameter=d_vox,
            porosity=por_share,
            allow_intersect=allow_intersect,
            segmented=True
        )
        print(f"[lognorm] sample {i+1}/{nsamples}, diam={d_nm:.1f} nm ({d_vox} vox), por={por_share:.3f}")
        ws.matrix = np.minimum(ws.matrix, tmp.matrix)
...
def build_psd_bins(specs: Dict[str, str], voxel_size_nm: float) -> List[Tuple[int, float]]:
    ...
    if bins_nm and weights and len(bins_nm) == len(weights):
        print("DEBUG bins_nm,weights:", bins_nm, weights)
...
def generate_volume(specs: Dict[str, str]) -> puma.Workspace:
    ...
    if generator == "fibers":
        ...
        ws = generate_random_fibers(shape=shape, radius=int(radius_vox),
                                    ...
                                    allow_intersect=True)
    elif generator == "lognormal":
        mu_log = float(specs.get("psd_lognorm_mu", 5.9))
        sigma_log = float(specs.get("psd_lognorm_sigma", 0.45))
        nsamples = int(specs.get("psd_nsamples", 8))
        d_min_nm = float(specs.get("psd_diam_min_nm", 40))
        d_max_nm = float(specs.get("psd_diam_max_nm", 1800))
        ws = lognormal_random_spheres(
            shape=shape,
            ...
            d_max_nm=d_max_nm,
            allow_intersect=allow_intersect,
            seed=int(specs.get("random_seed", 42)) if "random_seed" in specs else None
        )
    else:
