...
DEFAULT_OBJ_PATH = SCRIPT_DIR / "voxel_direct_syn1.obj"
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "demo_output"
...
def get_single_mesh_object():
    ...
    if len(mesh_objects) != 1:
        raise RuntimeError(f"Expected exactly one mesh object in scene, found {len(mesh_objects)}: {[obj.name for obj in mesh_objects]}")
...
def render(n, num_slices=2, flat=FLAT, obj_path=DEFAULT_OBJ_PATH, output_dir=DEFAULT_OUTPUT_DIR):
    ...
    # This path is intended for reconstruction meshes that need the historical 0.01 scale normalization.
    porous = get_single_mesh_object()
    apply_object_scale_to_mesh(porous, (0.01, 0.01, 0.01))
    porous.scale[1] *= -1
    ...
    for i in range(0, num_slices):
        ...
        bpy.data.worlds["World"].node_tree.nodes["Background"].inputs["Color"].default_value = (0.06, 0.06, 0.06, 1)
    ...
    params = {
        ...
        "scale_applied": True,
        "applied_scale_xyz": [0.01, 0.01, 0.01],
    }
